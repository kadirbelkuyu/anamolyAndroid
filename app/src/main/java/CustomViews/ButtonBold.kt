...
class ButtonBold : Button {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        val tf = Typeface.createFromAsset(context.assets, "fonts/Tajawal-Bold.ttf")
        typeface = tf

        if (attrs != null) {
            val a = context.theme.obtainStyledAttributes(attrs, R.styleable.ButtonBold, 0, 0)

            val textColor = if (a.getInt(R.styleable.ButtonBold_buttonType, 1) == 1)
                AppController.buttonTextColor
            else
                AppController.secondButtonTextColor

            val bgColor = if (a.getInt(R.styleable.ButtonBold_buttonType, 1) == 1)
