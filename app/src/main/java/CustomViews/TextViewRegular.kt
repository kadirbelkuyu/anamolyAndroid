...
class TextViewRegular : TextView {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            val a = context.theme.obtainStyledAttributes(attrs, R.styleable.TextViewRegular, 0, 0)

            if (a.getBoolean(R.styleable.TextViewRegular_allowBG, false)) {
                val gradientDrawable = GradientDrawable()
                gradientDrawable.cornerRadius =
                    a.getDimension(R.styleable.TextViewRegular_cornerRadius, 5F)
