...
class ButtonRegular : Button {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            val a = context.theme.obtainStyledAttributes(attrs, R.styleable.ButtonRegular, 0, 0)

            val textColor = if (a.getInt(R.styleable.ButtonRegular_buttonType, 1) == 1)
                AppController.buttonTextColor
            else
                AppController.secondButtonTextColor

            val bgColor = if (a.getInt(R.styleable.ButtonRegular_buttonType, 1) == 1)
